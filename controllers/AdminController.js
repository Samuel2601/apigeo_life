...
const login_admin = async function (req, res) {
	...
	try {
		const admin_arr = await Model.Usuario.find({ correo: data.correo });
			if (admin_arr.length != 0) {
				...
				if (user.estado != 'Off') {
					bcrypt.compare(data.password, user.password, async function (error, check) {
						if (check) {          
							...
							res.status(200).send({messages: 'Sin Coincidencia' , data: undefined });
						}
					});         
				} else {
					//messages= 'Su institución se encuentra Deshabilitada'
					res.status(200).json({messages: 'Credenciales Deshabilitadas' , data: undefined });					
				}
			}else{
				res.status(200).json({messages: 'No Registrado' , data: undefined });
			}
	} catch (error) {
		console.log(error);
		res.status(200).send({ messages: 'ERROR Server', data: undefined });
	}
};
